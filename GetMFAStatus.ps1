...
$domain = $domain.Name[0].split('.')[0]
$ExportCSV="C:\mcallen\MFADisabledUserReport_$(($domain)+"_"+(Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv"
$ExportCSVReport="C:\mcallen\MFAEnabledUserReport_$(($domain)+"_"+(Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv"
...
Get-MsolUser -All | foreach{
 ...
 if(($MethodTypes -ne $Null) -or ($MFAStatus -ne $Null) -and (-Not ($DisabledOnly.IsPresent) ))
 {
  ...
  $PrintedUser++
  $Result=@{'DisplayName'=$DisplayName;'UserPrincipalName'=$upn;'Department'=$Department;'MFAStatus'=$MFAStatus;'ActivationStatus'=$ActivationStatus;'DefaultMFAMethod'=$DefaultMFAMethod;'AllMFAMethods'=$Methods;'MFAPhone'=$MFAPhone;'MFAEmail'=$MFAEmail;'LicenseStatus'=$LicenseStat;'IsAdmin'=$IsAdmin;'AdminRoles'=$RolesAssigned;'SignInStatus'=$SigninStat}
  $Results= New-Object PSObject -Property $Result
  $Results | Select-Object DisplayName,UserPrincipalName,Department,MFAStatus,ActivationStatus,DefaultMFAMethod,AllMFAMethods,MFAPhone,MFAEmail,LicenseStatus,IsAdmin,AdminRoles,SignInStatus | Export-Csv -Path $ExportCSVReport -Notype -Append
 }

 #Check for MFA disabled user
 #($DisabledOnly.IsPresent) -and 
...
elseif((Test-Path -Path $ExportCSVReport) -eq "True")
{
 ...
 If ($UserInput -eq 6)
 {
  #$EnabledAndDisabled = $ExportCSVReport + $ExportCSV
